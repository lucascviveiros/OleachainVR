...
public class VG_EC_SteamHand : VG_ExternalController
{
	...
	private Matrix4x4 modifyPose(Matrix4x4 rawPose, int boneId)
	{
		if (boneId == 1) return Matrix4x4.TRS(rawPose.GetColumn(3),
			...
			Vector3.one);
		else return Matrix4x4.TRS(rawPose.GetColumn(3),
			Quaternion.LookRotation(
			m_handType == VG_HandSide.LEFT ? -rawPose.GetColumn(2) : rawPose.GetColumn(2),
			m_handType == VG_HandSide.LEFT ? -rawPose.GetColumn(0) : rawPose.GetColumn(0)),
			Vector3.one);
	...
	public override bool Compute()
	{
		...
		for (int boneId = 0; boneId < GetNumBones(); ++boneId)
			m_poses[boneId] = modifyPose(m_poses[boneId], boneId);
#endif
