...
public class AssembleVGArticulation : MonoBehaviour
{
    ...
    public AudioSource disassembleSoundEffect;
    public AudioSource assembleSoundEffect;
    ...
    void dessembleByJointChange()
    {
        foreach (VG_HandStatus hand in VG_Controller.GetHands())
        {
                ...
                && jointType != VG_JointType.FLOATING)
            {
                ...
                float joinState;
                VG_Controller.GetObjectJointState(transform, out joinState);
                if (joinState == 0.0f && (sensor_pos - hand.m_hand.position).magnitude > m_disassembleDistance)
                {
